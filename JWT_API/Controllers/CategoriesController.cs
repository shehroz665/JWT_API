...
namespace JWT_API.Controllers
{
    ...
    public class CategoriesController : ControllerBase
    {
        ...
        private readonly ICategory _categoryRepoistory;
        public CategoriesController(ApplicationContext db, LoggingInterface logging, ICategory categoryRepoistory)
        {
            _db = db;
            _logging = logging;
        ...
        //new api
        [HttpGet]
        public IActionResult GetAllCategories(int from = 1, int to = 10, string searchTerm = "")
        {
            try
            {
                var res = _categoryRepoistory.GetAllCategories(from, to, searchTerm);
                var response = _logging.Success("Category Fetched Successfully", 200, res);
                return Content(response, "application/json");
            }
            catch (Exception ex)
        ...
        [HttpGet("{id}")]
        public IActionResult GetBookCatData(int id)
        {
            try
            {
                var response = "";
                ...
                var res = _categoryRepoistory.GetBookCatData(id);
                var data = new
                {
                    data = res
                };
                response = _logging.Success("Category Fetched Successfully", 200, data);
                return Content(response, "application/json");
            }
            catch (Exception ex)
            {
        ...
        [HttpGet("{id}")]
        [Authorize(Policy = "Admin")]
        public ActionResult<Categories> GetCat(int id)
        {
            var response = " ";
            if (id == 0)
            {
            ...
            var categoryData = _categoryRepoistory.GetCategory(id);
            if (categoryData != null)
            {
                response = _logging.Success("Category Fetched Successfully", 200, categoryData);
                return Content(response, "application/json");
        ...
        public ActionResult<Categories> DeleteCategory(int id)
        {
            ...
            var categoryData = _categoryRepoistory.DeleteCategory(id);
            if (categoryData != null)
            {
                response = _logging.Success("Category Deleted Successfully", 200, categoryData);
                return Content(response, "application/json");
        ...
        public ActionResult<Categories> UpdateCategory(int id, [FromBody] Categories categoryObj)
        {
            ...
            var categoryData = _categoryRepoistory.UpdateCategory(id, categoryObj);
            if (categoryData != null)
            {
                response = _logging.Success("Category Updated Successfully", 200, categoryData);
                return Content(response, "application/json");
        ...
        public ActionResult<Categories> ChangeStatusCategory(int id, [FromBody] StatusDto statusDto)
        {
            ...
            var categoryData = _categoryRepoistory.ChangeStatusCategory(id,statusDto);
            if (categoryData != null)
            {
                response = _logging.Success("category Status Updated Successfully", 200, categoryData);
                return Content(response, "application/json");
        ...
        public ActionResult<Categories> CreateCategory([FromBody] Categories categoriesObj)
        {
            ...
            var categoryData =_categoryRepoistory.CreateCategory(categoriesObj);
            if (categoryData.CatId == -1)
            {
                response = _logging.Failure("Category Already exists", 400, null);
                return Content(response, "application/json");
            ...
            response = _logging.Success("Category Created Successfully", 201, categoryData);
            return Content(response, "application/json");
        }
